...
<head>
    ...
    <title>Royalx Core | Employees</title>
    <!-- Favicon -->
...
<body>
      	...
      	<div class="layout-container">
	        ...
	        <div class="layout-page">
	            ...
	            <div class="content-wrapper">
	                    ...
	                    <div class="row">
	                      	<div class="col-md-4">
	                        	<h5 class="py-2 mb-2">
                                   ...
                                   <span class="text-muted fw-light"><i class="tf-icon bx bx-chevrons-right" cursorshover="true"></i> Employees </span> 
                                </h5>
	                      	</div>
	                      	<div class="col-md-8">
                                <div class="pull-right">
                                    <button type="button" class="btn btn-secondary v-top btn-sync1" data-bs-toggle="modal" data-bs-target="#SyncModal" cursorshover="true" >
                                        <span class="tf-icons bx bx-sync" cursorshover="true"></span> Sync Innovix Data
                                    </button>
                                    ...
                                    <div class="btn-group v-top">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end" style="">
                                            <li><a class="dropdown-item filtered active" href="javascript:void(0);" title="all">All Employees</a></li>
		                        ...
		                        <div class="table-responsive text-nowrap">
		                          	<table class="table">
		                                ...
		                                <tr>
                                            ...
                                            <th class="text-muted unset">SystemUser</th>
	            	...
	            	<input type="hidden" id="json-employees" value="{{ url('json/employees') }}">
    ...
    <div class="modal modal-top modal-lg fade" id="SyncModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTopTitle">Are you sure to sync data from InnovixData?</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    Employee data download from InnovixData table.
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-label-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-secondary btn-sync">Sync</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- Edit Modal -->
    <div class="modal modal-top fade" id="EditEmployee" tabindex="-1">
       	<div class="modal-dialog">
            <form class="modal-content">
                <div class="modal-header">
                  	<h5 class="modal-title" id="modalTopTitle">Edit Employee</h5>
                  	<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  	<div class="row">
                    	<div class="col mb-3">
	                      	<label for="old-id" class="form-label">Employee ID <span class="text-danger">*</span></label>
	                      	<input type="text" id="old-id" class="form-control" placeholder="REX-2023-3710">
	                    </div>
                  	</div>
                  	<div class="row">
	                    <div class="col mb-3">
	                      	<label for="old-name" class="form-label">Employee Name <span class="text-danger">*</span></label>
	                      	<input type="text" id="old-name" class="form-control" placeholder="Daw Thet Hnin May">
	                    </div>
                  	</div>
                  	<div class="row g-2">
	                    <div class="col mb-3">
	                      	<label for="old-position" class="form-label">Position <span class="text-danger">*</span></label>
	                      	<input type="text" id="old-position" class="form-control" placeholder="Customer Service Staff">
	                    </div>
	                    <div class="col mb-3">
	                      	<label for="old-department" class="form-label">Department <span class="text-danger">*</span></label>
	                        <select id="old-department" class="select2 form-select form-select-lg" data-allow-clear="true">
	                            @foreach($branches as $branch)
	                            <option value="{{ $branch->BranchNameEn }}">{{ $branch->BranchNameEn }}</option>
	                            @endforeach
	                        </select>
	                    </div>
                  	</div>
                  	<div class="row g-2">
                    	<div class="col mb-3">
                      		<label for="old-assign-department" class="form-label">Assign Department</label>
	                        <select id="old-assign-department" class="select2 form-select form-select-lg" data-allow-clear="true">
	                            <option value="HNO">HNO</option>
	                            <option value="HO">HO</option>
	                            <option value="MDY">MDY</option>
	                            <option value="NPT">NPT</option>
	                            <option value="SBR">SBR</option>
	                            <option value="YGN 1">YGN 1</option>
	                            <option value="YGN 2">YGN 2</option>
	                        </select>
                    	</div>

	                    <div class="col mb-3">
	                      	<label for="old-phone" class="form-label">Phone No</label>
	                      	<input type="text" id="old-phone" class="form-control" placeholder="09796988823,09671132468">
	                    </div>
                  	</div>
                  	<span class="text-danger">*</span> is required fields.
                </div>
                <div class="modal-footer">
                  	<button type="button" class="btn btn-label-secondary" data-bs-dismiss="modal">Close</button>
  	...
  	<script type="text/javascript">
    	$(document).ready(function(){
                      ...
                      if(data.total > 0){
                    	 $.each(data.data, function (i, item) {
                       		$("#fetched-data").append('<tr>'
                                +'<td class="'+(item.LastDayOfWork ==null? '':'strike' )+'"><code class="h6 text-secondary fw-semibold">'+item.EmployeeId+'</code></td>'
                                +'<td>'+item.EmployeeName+'</td>'
                                +'<td>'+item.Position+'</td>'
                                +'<td>'+(item.Sync == 1? '<span class="badge badge-center rounded-pill bg-success"><i class="bx bx-check"></i></span>':'<span class="badge badge-center rounded-pill bg-danger"><i class="bx bx-x"></i></span>')+'</td>'
                                +'<td>'+(item.SystemUser == 1? '<i class="menu-icon tf-icons bx bx-check-shield text-success"></i>':'<i class="menu-icon tf-icons bx bx-shield-x text-danger"></i>')+'</td>'
                                +'<td>'
                                    +'<div class="btn-group">'
                                        ...
                                        +'<ul class="dropdown-menu dropdown-menu-end" style="">'
                                            +'<li ><a href="'+url+'/admin/employees/'+item.Id+'" class="dropdown-item text-success  load-id" cursorshover="true"><span class="tf-icons bx bx-search" cursorshover="true"></span> View Employee</a></li>'
                                            +'<li><a class="dropdown-item text-danger  load-id" href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#DeleteTownship" cursorshover="true" title= '+item.Id+'><span class="tf-icons bx bx-trash" cursorshover="true"></span> Delete Employee</a></li>'
                                        +'</ul>'
                                    +'</div>'
                                +'</td>'
                            +'</tr>');
            ...
            $('.form-control').on("keyup",function search(e) {
                ...
                if($("#create-id").val().trim().length === 0 ||
                    $("#create-name").val().trim().length === 0 ||
                    $("#create-position").val().trim().length === 0
                ) {
                ...
                if($("#old-id").val().trim().length === 0 ||
                    $("#old-name").val().trim().length === 0 ||
                    $("#old-position").val().trim().length === 0
                ) {
            ...
            $('body').delegate(".btn-sync","click",function () {
                ...
                $.ajax({
                    url: url+'/admin/synced/employees',
                    type: 'GET',
                    data: {},
                    success: function(data){
